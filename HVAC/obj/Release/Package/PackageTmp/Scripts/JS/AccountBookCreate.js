function SaveAccount() {
    ...
    var totalamount = parsenumeric($('#TotalAmt'));

    if (itemcount == 0 || parseFloat(totalamount) == 0) {
        Swal.fire('Data Validation', 'Add Items in the Detail Grid!', 'error');
        return false;
    }
    ...
    for (var i = 0; i < itemcount; i++) {
        ...
        var item = {
            ...
            EmployeeId: $('#hdnEmployeeID_' + i).val(),
...
(function ($) {
    ...
    function initformControl() {
        ...
        function GetAChead(empId) {
            debugger;

            $.ajax({
                url: '/Accounts/GetACHeadEmployee',
                type: 'GET',
                datatype: "Json",
                data: { empId: empId },
                success: function (data) {
                    if (data != null) {
                        var acHeadID = data.ACHeadID;
                        var acHeadName = data.ACHead;
                        //selectreceivedfrom


                        $("#SelectedReceivedFrom").val(acHeadID);
                        $("#ReceivedFrom").val(acHeadName);
                    }
                    else {
                        $("#SelectedReceivedFrom").val(0);
                        $("#ReceivedFrom").val('failed');
                    }
        ...
        $("#btnAdd").click(function () {
            ...
            if ($('#chkEmp').prop('checked')) {
                ...
                else if ($("#DDEmployee option:selected").val() == "") {
                    alert('Please Select "Employee Name"');
                    $("#DDEmployee").focus();
                    return false;
                }
                else if ($("#DDEmployee").val() != "") {
                    empId = $('#DDEmployee').val();
            ...
            if ($("#amount").val() == "") {
                ...
                if ($('#chkEmp').prop('checked')) {
                    textboxvalue = $("#DDEmployee option:selected").text();
                    Employee = '<input type="hidden" id="hdnEmployeeID_' + i + '" value="' + $("#DDEmployee option:selected").val() + '" />';
                    $("#ReceivedFrom").val(textboxvalue);
                    //Employee = '<input type="hidden" id="hdnEmployeeID_' + i + '" value="' + $("#DDEmployee option:selected").val() + '" />';
                ...
                var objHtml = '<tr>' + '<td style="padding-left:10px">' + Employee + '<input type = "hidden" value="false" id="hdndeleted_' + i + '" name="AcJDetailVM[' + i + '].IsDeleted" class="hdndeleted" /><input type="hidden" class=JAcHead id="JAcHead_' + i + '" value="' + $("#SelectedReceivedFrom").val() + '" name="AcJDetailVM[' + i + '].AcHeadID"><input type="text" class="form-control clsreceivedfrom" id="ReceivedFrom_' + i + '" value="' + $("#ReceivedFrom").val() + '" name="AcJDetailVM[' + i + '].AcHead" disabled></td><td style="padding-left:10px">' + amounttaxcheck + '<input type="hidden" value="' + $("#AmountIncludingTax").is(':checked') + '" name="AcJDetailVM[' + i + '].AmountIncludingTax"></td><td class="textright"  style="padding-left:10px"><input    id="JAcAmount_' + i + '" type="text" class="form-control JAcAmt text-right txtNum" value="' + parseFloat($("#amount").val()).toFixed(_decimal) + '" name="AcJDetailVM[' + i + '].Amt"></td>' + '<td class=textright style="padding-right:10px"><input type="text"    id="JAcTaxPercent_' + i + '" class="form-control JAcAmt textright txtNum" value="' + $("#TaxPercent").val() + '" name="AcJDetailVM[' + i + '].TaxPercent"></td>' + '<td class="textright" style="padding-right:10px"><input type="text" class="JAcAmt textright form-control txtNum" readonly="readonly" id="JAcTaxAmount_' + i + '" value="' + parseFloat($("#TaxAmount").val()).toFixed(3) + '" name="AcJDetailVM[' + i + '].TaxAmount"></td>';
                objHtml = objHtml + Projecthtml + '<td><input type="text" class="form-control" id="editremark_' + i + '" value="' + $("#remarks").val() + '"  name="AcJDetailVM[' + i + '].Rem" /></td>';
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $.each(data, function (index, value) {
                    ...
                    if (value.ProjectID != 0 || value.ProjectID != null) {
                        Projecthtmledit = '<td> <input class="text-right form-control"  type="text" readonly id="txtproject_' + i + '" value="' + value.ProjectName + '" /> <input type="hidden" id="hdnPro_' + i + '" value="' + value.ProjectID + '"/></td>'

                    }
                    else { Projecthtmledit = '<td></td>'; }
                    if (value.EmployeeId != 0 || value.EmployeeId != null) {



                        //Employee = '<input type="hidden" id="hdnEmployeeID_' + i + '" value="' + $("#DDEmployee option:selected").val() + '" />';
                        Employeestag = '<input type="text" class="form-select" id="ReceivedFrom_' + i + '" value=" ' + value.EmpName + '" name="AcJDetailVM[' + i + '].AcHead" disabled><input type="hidden" id="hdnEmployeeID_' + i + '" value="' + value.EmployeeId + '" />';

                    }
                    if (value.EmployeeId == 0 || value.EmployeeId == null) {
                        Employeestag = '<input type="text" class="form-select" id="ReceivedFrom_' + i + '" value=" ' + value.AcHead + '" name="AcJDetailVM[' + i + '].AcHead" disabled>';
                    }

                    var objHtml = '<tr>' + '<td style="padding-left:10px"><input type ="hidden" id="hdndeleted_' + i + '" name="AcJDetailVM[' + i + '].IsDeleted" value="' + value.IsDeleted + '" class="hdndeleted" /> <input type="hidden" class=JAcHead id="JAcHead_' + i + '" value=' + value.AcHeadID + ' name="AcJDetailVM[' + i + '].AcHeadID">' + Employeestag + '<td style="padding-left:10px">' + amounttaxcheck + '</td>' + '<td  style="padding-right:50px"><input type="text"   id="JAcAmount_' + i + '" class="form-control textright JAcAmt textright form-control" value=' + parseFloat(value.Amt).toFixed(_decimal) + ' name="AcJDetailVM[' + i + '].Amt"></td>' + '<td class=textright style="padding-right:10px"><input type="text" id="JAcTaxPercent_' + i + '" class="JAcAmt textright form-control" value=' + parseFloat(value.TaxPercent).toFixed(2) + ' name="AcJDetailVM[' + i + '].TaxPercent"></td>' + '<td class="" style="padding-right:10px"><input class="textright form-control" type="text" readonly="readonly"  id="JAcTaxAmount_' + i + '" class="JAcAmt textright form-control" value=' + parseFloat(value.TaxAmount).toFixed(_decimal) + ' name="AcJDetailVM[' + i + '].TaxAmount"></td>';
                    objHtml = objHtml + Projecthtmledit+ '<td><input class="form-control" type="text" id="editremark_' + i + '"   title="' + value.Rem + '" value="' + value.Rem + '" name="AcJDetailVM[' + i + '].Rem" /></td>';
    ...
    $(document).ready(function () {
        ...
        $('.docs-datepicker-trigger').click(function () {
            var datefor = $(this).attr('datefor');
            $('#' + datefor).datepicker('show');
        })
