...
function SaveCounterLogin() {
    ...
    $.ajax({
        type: "POST",
        url: "/ShiftEntry/SaveCounterLogin/",
        datatype: "Json",
        data: {vm:model },
...
(function ($) {
    ...
    function initformControl() {
        ...
        if ($('#CounterLoginID').val() > 0) {
            $('#btnsave').val('Update');
            $('#OpeningBalance').trigger('change');
            if ($('#SalesClosed').val() == "True") {
                $('#btnsave').attr('disabled', 'disabled');
                Swal.fire('Data Validation', 'Closed Sales Could not be edited!', 'warning');
               
                $('#SalesDate').attr('disabled', 'disabled');
                $('#OpeningBalance').attr('disabled', 'disabled');
                $('#CounterID').attr('disabled', 'disabled');
