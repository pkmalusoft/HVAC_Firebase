...
function DeleteEnquiryClient(ID, index1) {
    ...
    Swal.fire({ title: "Are you sure?", text: "You won't be able to revert this!", icon: "warning", showCancelButton: !0, confirmButtonColor: "#34c38f", cancelButtonColor: "#f46a6a", confirmButtonText: "Yes, delete it!" }).then(
        function (t) {
            if (t.value) {
                $.ajax({
                    type: "POST",
                    url: '/Enquiry/DeleteEnquiryClient',
                    datatype: "html",
                    data: {
                        'ID': ID,'EnquriyNo' : $('#EnquiryNo').val()
                    },
                    success: function (data) {
...
function reloadlog() {
    $.ajax({
        type: 'POST',
        url: '/Enquiry/ShowLogList',
        datatype: "html",
        data: {
            EnquiryNo: $('#EnquiryNo').val()
        },
        success: function (data) {
            $("#enquiry-log").html(data);
        }
    });
}
...
(function ($) {
    ...
    function init() {
        ...
        setTimeout(function () {
            if ($('#EnquiryID').val() > 0) {
                ...
                if ($("#EnquiryNo").attr('readonly')!='readonly')
                    $("#EnquiryNo").focus();
                // Clear old options and add empty default
                $('#drpClientID').empty().append('<option value="">-- Select Client --</option>');
    ...
    $(document).ready(function () {
        ...
        $('#confirmStatusBtn').on("click", function (e) {
            let statusName = $('#hdnstatuschange').attr('statusname');
            let statusId = $('#hdnstatuschange').attr('statusid');
            let enquiryId = $("#EnquiryID").val(); // hidden input in the view

            $('#statusConfirmModal').modal('hide');
                   $.ajax({
                            type: "POST",
                            url: "/Enquiry/SaveEnquiryStatus", // or use global var for Url.Action
                            dataType: "json",
                            data: {
                                StatusID: statusId,
                                StatusName: statusName,
                                EnquiryID: enquiryId,
                                Reason: $('#StatusRemarks').val()
                            },
                            success: function (data) {
                                if (data.Status === "ok") {
                                    Swal.fire("Enquiry Status", "Status Changed Successfully", "success");

                                    // Update main button text
                                    $("#btnStatus").html("Status : " + statusName);

                                    // Update hidden field if present
                                    $("#EnquiryStatusID").val(statusId);

                                    if (typeof reloadlog === "function") reloadlog();
                                } else {
                                    Swal.fire("Enquiry Status", data.Message, "error");
                                }
                            },
                            error: function () {
                                Swal.fire("Error", "Something went wrong while saving status.", "error");
                            }
                        });
            ...
            $(".status-item").on("click", function (e) {
                ...
                $('#hdnstatuschange').attr('statusid', statusId);
                $('#hdnstatuschange').attr('statusname', statusName);

                Swal.fire({
                    title: "Are you sure?",
                    text: "Do you want to update the enquiry status?",
                    icon: "warning",
                    showCancelButton: true,
