...
namespace HVAC.Controllers
{
    ...
    public class SupplierController : Controller
    {
        ...
        public ActionResult Details(int id = 0)
        {
            try
            {
                var supplier =  db.SupplierMasters.FindAsync(id);
                if (supplier == null)
                {
                    return HttpNotFound();
                }
        ...
        public ActionResult Create()
        {
                ...
                var supplierMasterTypes = (from d in db.SupplierTypes where (d.SupplierTypeID==1 || d.SupplierTypeID==2 || d.SupplierTypeID==3) select d).ToList();
        ...
        public ActionResult Create(SupplierMaster supplier)
        {
            try
            {
                ...
                {
                    var supplierMasterTypes1 = (from d in db.SupplierTypes select d).ToList();
                    ViewBag.SupplierType = supplierMasterTypes1;
                    return View(supplier);
                }
        ...
        public JsonResult GetSupplierName(string term)            
        {
            if (term!=null && term!="")
            {
                var _clientlist = (from c in db.SupplierMasters where c.SupplierName.ToLower().Contains(term.ToLower()) orderby c.SupplierName select new { SupplierID = c.SupplierID, SupplierName = c.SupplierName }).ToList();
                return Json(_clientlist, JsonRequestBehavior.AllowGet);
            }
            else
            {
                var _clientlist = (from c in db.SupplierMasters orderby c.SupplierName select new { SupplierID = c.SupplierID, SupplierName = c.SupplierName }).ToList();
                return Json(_clientlist, JsonRequestBehavior.AllowGet);
            }
