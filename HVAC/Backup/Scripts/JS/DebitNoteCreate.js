                    ...
                    if (response.status == "OK") {
                        ...
                        setTimeout(function () {
                            if ($('#CreditNoteID').val() == 0) {
                                location.href = '/DebitNote/Create?id=0';
                            }
                            else {
                                location.href = '/DebitNote/Create?id=' + response.DebitNoteID;
                            }
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            $.ajax({
                ...
                data: {
                    'ID': ID, 'amountreceived': parseFloat(TotalAmount), 'RecPayId': recpayid, 'SupplierTypeId':$('#SupplierTypeId').val(), 'EntryType': "DN"
                },
                success: function (response) {
                    debugger;
    ...
    function initformControl() {
        ...
        $('#TransType').change(function () {
            if ($('#DebitNoteId').val() == 0) {
                $.ajax({
                    type: "GET",
                    url: '/DebitNote/GetDebitNoteNo/',
                    data: { 'Type': $('#TransType').val(),'SupplierTypeID' : $('#SupplierTypeId').val()},
                    success: function (response) {
                        $('#DebitNoteNo').val(response.DebitNoteNo);
                        $('#AcHeadID').val(response.AcHeadID);
                        $('#AcHeadName').val(response.AcHeadName);
                        $('#lblAcHeadName').html(response.AcHeadName);
                    }
                });
            }
        });
        ...
        setTimeout(function () {
            if ($('#DebitNoteId').val() > 0) {
                ...
                $("#SupplierTypeId").val($("#SupplierTypeId").attr('svalue')).trigger('change');
                $('#btnsave').val('Update');
                ...
                $('#TransType').val('DN').trigger('change');
