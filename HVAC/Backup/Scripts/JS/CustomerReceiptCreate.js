...
function PaymentModeChange() {
    ...
    if (val == 1) { //Bank
        $('#paymentname').html('Bank Head Name');
        ...
        $('#paymentname').html('Cash Head Name');
        $("#ChequeNo").val('');
        $("#ChequeDate").val('');
...
function SaveCustomerReceipt() {
    debugger;
    ValidateTotal();
    //var amount = parsenumeric($('#Amount'));
    //if (amount == 0) {
    //    savevalid = false;
    //}
    //else {
        ...
        if (itemcount > 0) {
            for (i = 0; i < itemcount; i++) {
                ...
                if (i == (itemcount - 1)) {
                    $.ajax({
                        type: "POST",
                        url: "/CustomerReceipt/SaveCustomerReceipt",
                        datatype: "Json",
                        ...
                        success: function (response) {
                            if (response.status == "OK") {
                                ...
                                setTimeout(function () {
                                    location.href = '/CustomerReceipt/Index'; //ate?id=' + response.RecPayID;
                                }, 200)
            ...
            $.ajax({
                ...
                success: function (response) {
                    if (response.status == "OK") {
                        ...
                        Swal.fire("Save Status", response.message, "success");
                        setTimeout(function () {
                            location.href = '/CustomerReceipt/Create?id=' + response.RecPayID;
                        }, 200)
    ...
    if (itemcount > 0) {
        $('[id^=' + idtext + ']').each(function (index, item) {
            ...
            if ((index + 1) == itemcount) {
                ...
                getTotalAdjustment(rowindex);
                setTimeout(function () {
                    var balance = parsenumeric($('#txtbalance_' + $('#ParentRowCount')));
                    var allocate = parsenumeric($('#txtinvoice_' + $('#ParentRowCount')))
